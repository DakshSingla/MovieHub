...
const SeatLayout = () => {
  ...
  const bookTickets = async () => {
    try {
      ...
      // Safely obtain a token; if acquiring token fails, prompt sign-in
      let token;
      try {
      ...
      if(data && data.success){
        setLastBookingId(data.booking?._id || data.bookingId);
    ...
    <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50'>
      ...
      <div className='flex-1 flex flex-col items-center'>
        ...
        <DemoPaymentModal
          ...
          amount={selectedSeats.length * (show?.price || 0)}
          onSuccess={async () => {
            if (lastBookingId) {
              try {
                await axios.post(`/api/booking/fakepay/${lastBookingId}`);
