...
// API to get the latest added movie
export const getLatestMovie = async (req, res) => {
    try {
        const latestMovie = await Movie.findOne().sort({ createdAt: -1 });
        if (!latestMovie) {
            return res.json({ success: false, message: "No movies found" });
        }
        res.json({ success: true, movie: latestMovie });
    } catch (error) {
        console.error(error);
...
//API TO GET NOW PLAYING MOVIES FROM TMDB APP
...
//Apt to add new show to database
...
export const getShows = async (req , res) => {
    try{
        ...
        console.log(`getShows: returning ${responseShows.length} unique movies`);
        res.json({success : true , shows: responseShows })
    } catch (error){
         console.error(error);
         res.json({success : false , message: error.message});
    }
